...
namespace CaclulatorDemo
{
    public partial class Form1 : Form
    {
        ...
        private void dateTimePicker1_ValueChanged(object dtmp, EventArgs e)
        {
            if (Equal == true)
            {
                DTMTxt.Clear();
            ...
            DTMTxt.Text += (dtmp as DateTimePicker).Text;
        }

        private void DTMMinus_Click(object btn, EventArgs e)
        {
            DTMTxt.Text += (btn as Button).Text;
        }

        private void DTMEqual_Click(object sender, EventArgs e)
        {
            Equal = true;
            int idx = DTMTxt.Text.LastIndexOf('-'); // To handle ex: -9-2
            Console.WriteLine(idx);
            if (idx > 0)
            {
                var op1 = Convert.ToDateTime(DTMTxt.Text.Substring(0, idx));
                var op2 = Convert.ToDateTime(DTMTxt.Text.Substring(idx + 1));
                DTMTxt.Text = (op1 - op2).ToString();
            }
        }
    }
}
